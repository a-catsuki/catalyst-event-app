...
class FlareSupabaseUser extends BaseAuthUser {
  FlareSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> flareSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FlareSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
